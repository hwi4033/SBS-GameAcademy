namespace Class5th__Arrangement_
{
    internal class Program
    {
        ...
        {
            #region 접근 지정자
            // 구조체 내부에 포함되어 있는 속성에 접근 범위를 제한하는 지정자입니다.
        ...
        static void Main(string[] args)
        {
            ...
            // Console.WriteLine();
            // 
            // items = new int[5];     // 기존 메모리 공간에 연속적으로 공간을 만들지 않고 새로운 메모리 공간을 생성한다.
            // 
            // for (int i = 0; i < items.Length; i++)
            // {
            //     Console.Write(items[i] + " ");
            ...
            // int[] number = new int[] { 7, 22, 11, 18, 3 };
            // 
            // int max = number[0], min = number[0];       // 첫 번째 인덱스 값을 할당하고 반복문을 통해 나머지 인덱스의 값을 비교
            // 
            ...
            // 문자열은 공백도 함께 메모리 공간에 포함하여 크기가 결정되며, 마지막에는 문자열의 끝을 알려주는 제어문자(NULL)가 추가됩니다.

            // name = "Marine";

            // name[3] = 'A';   (X) C#에서는 읽기 전용이다.

            // Console.WriteLine("name 변수의 값 : " + name);

            // 문자열의 경우 세러 연속적인 메모리 공간으로 연결되어 있지만, 문자 배열 사이에 무효의 문자를 넣게 되면 무효의 문자까지만 문자열을 출력합니다.
            #endregion
            ...
            // Console.WriteLine("Damage 함수가 반환하는 값 : " + Damage());
            ...
            // 프로그램이 시작하고 종료될 때까지 메모리에 남아있는 특징을 가지고 있습니다.
            #endregion

            #region DATA 영역
            // 전역 변수와 정적 변수가 저장되는 영역으로 프로그램의 시작과 함께 할당되며,
            // 프로그램이 종료될 때까지 메모리에 남아있는 특징을 가지고 있습니다.
            #endregion
            ...
            // 스택영역에 저장되는 함수의 호출 정보를 스택 프레임이라고 합니다.
            #endregion
