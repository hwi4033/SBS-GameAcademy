using InputSystem;  // namespace가 다른 cs 파일 가져오기
...
namespace Class8th__Polymorphism_
{
    internal class Program
    {
        ...
        public enum State
        {
            ...
            // 열거형에서 값을 특정한 위치에 새로 할당하게 되면 그 다음의 값은 새로 할당한 값의 + 1이 됩니다.
        }
        ...
        static void Main(string[] args)
        {
            ...
            #region 함수의 오버로딩
            // 같은 이름의 함수를 매개변수의 자료형과 매개변수의 수로 구분하여 여러 개를 선언할 수 있는 기능입니다.
            ...
            // 함수의 오버로딩의 경우 함수의 매개변수에 전달하는 인수의 형태를 보고 호출하므로, 반환형으로 함수의 오버로딩은 생성할 수 없습니다.
            #endregion
            ...
            // marine.Skill();     // 상위 클래스의 Skill함수는 출력이 안된다.

            // Unit unit = new Ghost();

            // unit.Skill();    // 상위 클래스의 SKill함수가 출력된다.
            ...
            // 가상 함수 테이블을 통해 DATA 영역의 함수를 실행한다.
            ...
            #region 열거형
            // 여러 개의 상수를 하나의 컨테이너에서 관리하기 위해 사용하는 자료형입니다.

            // State state = State.IDLE;
            // State state = (State) 0;     // 형변환을 통해 정수값을 받을 수 있다.
            ...
            #region 콘솔 입력
            // 입력과 출력 사이에 버퍼가 존재한다. 버퍼에서 수정이 가능

            // string name = Console.ReadLine();

            // Console.WriteLine("name 변수의 값 : " + name);

            // int count = Console.Read();     // 아스키 코드값으로 출력한다.
